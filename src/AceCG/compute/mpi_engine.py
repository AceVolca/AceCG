...
def _shared_energy_mask_from_steps(
    ...
    raw_specs: list[Any] = []
    if "energy_mask" in shared_spec:
        raw_specs.append(shared_spec.get("energy_mask"))
    raw_specs.extend(step.get("energy_mask") for step in steps if "energy_mask" in step)
    normalized = []
    for raw in raw_specs:
        item = normalize_energy_mask_spec(raw)
        if item:
    ...
    for item in normalized[1:]:
        if item != first:
            raise ValueError("All post-processing steps must share the same energy_mask.")
    return first
    ...
    def compute(
        ...
        **kwargs,
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            # ── Phase 1: neighbor-pair cache ─────────────────────────────
            # ``chunk`` mode rebuilds neighbors for each noisy/sample chunk;
            # other modes reuse the first valid cache to avoid repeated pair
            # searches when the configured skin/reference frame is sufficient.
            rebuild_neighbor = (
                build_pairs
            ...
            # ── Phase 2: frame geometry ─────────────────────────────────
            # Geometry is the shared payload boundary: energy, force, and
            # reducer hooks consume this object instead of rebuilding topology
            # or neighbor information independently.
            t0 = time.monotonic()
            ...
            # ── Phase 3: energy observables ─────────────────────────────
            # Energy requests are accumulated immediately with normalized
            # sample weights so downstream reducers always see frame averages
            # unless a reducer explicitly requested stacked outputs.
            ...
            # ── Phase 4: force observables ──────────────────────────────
            # Force-matching statistics use absolute frame weights inside the
            # force kernel, while force values and Jacobians keep the same
            # reducer-facing average convention as energy observables.
            ...
            # Batched CDFM reinforcement statistics need cross terms before
            # reducer-facing averages discard per-sample alignment.
            if (
                is_batch
                    ...
                    for item in gathered:
                        for subkey, value in dict(item).items():
                            if isinstance(value, Mapping):
                                existing = dict(merged.get(subkey, {}))
                                for inner_key, inner_value in value.items():
                                    existing[inner_key] = (
                                        existing.get(inner_key, 0)
                                        + int(inner_value)
                                    )
                                merged[subkey] = existing
                                continue
...
if __name__ == "__main__":
    # The module entry point owns MPI COMM_WORLD initialization for post jobs;
    # library callers inject a communicator explicitly or run in serial mode.
    import sys
