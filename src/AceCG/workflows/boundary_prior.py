...
def read_boundary_summary(path: str | Path) -> dict[str, dict[str, float | None]]:
    """Read PMF boundary rows from a diagnostics Markdown table."""
...
def boundary_prior_spec_from_config(config: ACGConfig) -> dict[str, Any]:
    """Resolve boundary-prior bounds from config extras or a summary file."""
    ...
    if raw_path is None:
        raise ValueError(
            "Set training.boundary_prior or training.boundary_prior_path for "
            "the boundary-prior workflow."
        )
...
def run_boundary_prior(config: ACGConfig) -> dict[str, Any]:
    """Apply endpoint priors to the configured forcefield and write tables."""
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        ...
        description="Apply one-shot endpoint priors to an AceCG forcefield.",
        allow_abbrev=False,
    )
