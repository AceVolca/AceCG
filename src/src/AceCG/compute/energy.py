...
def energy(
    ...
    *,
    ...
    ``'energy_grad_outer'``, ``'gauge_free_energy_grad'``,
    and ``'gauge_free_energy_grad_outer'`` as requested. Unrequested keys
    are absent.
    """
    need_gauge_free_energy_grad_buffer = bool(
    ...
    if not (
        ...
        or need_gauge_free_energy_grad_buffer
    ):
        return {}
    ...
    gauge_free_energy_grad = (
        ...
        if need_gauge_free_energy_grad_buffer
        else None
    )
    if return_hessian:
    ...
    for key, pot, sl in param_blocks:
        ...
        if key.style in {"bond", "angle"} and getattr(pot, "bonded", False):
            support_min = getattr(pot, "_support_min", None)
            support_max = getattr(pot, "_support_max", None)
            if support_min is not None and np.any(rv < float(support_min)):
                raise ValueError(
                    f"Bonded samples for {key.label()} fell below spline support {float(support_min):.8g}."
                )
            if support_max is not None and np.any(rv > float(support_max)):
                raise ValueError(
                    f"Bonded samples for {key.label()} exceeded spline support {float(support_max):.8g}."
                )
            if cutoff is not None and np.isfinite(cutoff):
                if np.any(rv > float(cutoff)):
                    raise ValueError(
                        f"Bonded samples for {key.label()} exceeded cutoff {float(cutoff):.8g}."
                    )
        ...
        if gauge_free_energy_grad is not None:
            gauge_free_energy_grad[sl] += pot.gauge_free_energy_grad_sum(rv)
    ...
    if return_gauge_free_energy_grad_outer:
        if gauge_free_energy_grad is None:
            raise RuntimeError("Gauge-free gradient outer product requested without a gradient buffer.")
        result["gauge_free_energy_grad_outer"] = np.outer(
            gauge_free_energy_grad,
            gauge_free_energy_grad,
        )
