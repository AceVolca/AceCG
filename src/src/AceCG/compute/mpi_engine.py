...
def _frame_weight_mapping_from_spec(
    ...
    """Convert ``spec['frame_weight']`` into a full-trajectory weight sequence.

    ``analysis.rdf`` in the active AceCG tree normalizes weights as a 1D array
    indexed by global frame id. The incoming spec stores weights only for the
    selected frame set, so this helper expands them to a full-trajectory
    sequence and leaves unspecified frame ids at weight 1.
    """
    ...
    expanded = np.ones(int(total_frames), dtype=np.float64)
    expanded[np.asarray(selected_ids, dtype=np.int64)] = weights
...
class MPIComputeEngine:
    ...
    def compute(
        ...
        frame: Tuple[int, np.ndarray, np.ndarray, Optional[np.ndarray]],
        topology_arrays: TopologyArrays,
        forcefield_snapshot: Forcefield,
        frame_weight: float = 1.0,
        interaction_mask: Optional[Dict[InteractionKey, bool]] = None,
        pair_type_list: Optional[Sequence[InteractionKey]] = None,
        ...
        **kwargs,
        ...
        pair_cache = None
        if build_pairs:
            assert pair_type_list is not None
            assert pair_cutoff is not None
            t0 = time.monotonic()
            pair_cache = compute_pairs_by_type(
                positions=positions,
                box=box,
                pair_type_list=list(pair_type_list),
        ...
        if (
            ...
            or request["need_gauge_free_energy_grad"]
            or request["need_gauge_free_energy_grad_outer"]
        ):
            t0 = time.monotonic()
            results.update(
                energy(
                    ...
                    return_gauge_free_energy_grad=request["need_gauge_free_energy_grad"],
                    return_gauge_free_energy_grad_outer=request["need_gauge_free_energy_grad_outer"],
                )
            )
            if timing is not None:
    ...
    def _reduce_step_partials(
        ...
        *,
        ...
        if comm is not None and size > 1 and rank != 0:
            return None

        if reduced is None:
            raise RuntimeError("_reduce_step_partials() did not produce a root result.")
    ...
    def _preprocess_cdfm_zbx_steps(
        ...
        *,
        ...
        forcefield_snapshot: Forcefield,
        topology_arrays: TopologyArrays,
        pair_type_list: Sequence[InteractionKey],
        ...
        for step in one_pass_steps:
            ...
            if rank == 0:
                ...
                init_atoms = init_universe.atoms
                if init_atoms is None:
                    raise RuntimeError(
                        f"cdfm_zbx init topology {init_topology!r} did not load atoms"
                    )
                init_positions = np.asarray(
                    init_atoms.positions, dtype=np.float64
                )
                init_box = np.asarray(
    ...
    def run_post(
        ...
        if topology_arrays is None:
            raise RuntimeError("MPIComputeEngine.run_post() requires local topology arrays.")
        if sel_indices is None:
            raise RuntimeError("MPIComputeEngine.run_post() requires a local atom selection.")
        if total_frames is None:
            raise RuntimeError("MPIComputeEngine.run_post() requires the total frame count.")
        ...
        if rank == 0 and rdf_steps:
            rdf_frame_weights = _frame_weight_mapping_from_spec(shared_spec, total_frames)
