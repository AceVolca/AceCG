"""Define staged AceCG trainer interfaces."""
...
class BaseTrainer(ABC):
    ...
    def is_gauge_free_energy_grad_cacheable(self) -> bool:
        """Report whether all active gauge-free channels are safe to AA-cache.

        Returns
        -------
        bool
            ``True`` if every active parameter is marked cacheable by its
            potential's gauge-free contract; ``False`` otherwise.
        """
        n_params = self.forcefield.n_params()
        ...
        offset = 0
        for _, pot in IteratePotentials(self.forcefield):
            cacheable_mask = np.asarray(pot.is_gauge_free_energy_grad_cacheable(), dtype=bool)
            n_local = pot.n_params()
