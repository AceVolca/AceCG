...
class REMTrainerAnalytic(BaseTrainer):
    ...
    def make_batch(
        ...
        ----------
        AA_dist
            Per-frame AA features (e.g., Pair2DistanceByFrame). Passed to
            ``dUdLByFrame(potential, AA_dist)`` (or ``dUdL_parallel`` if enabled).
        CG_dist
    ...
    def step(self, batch: REMBatch, apply_update: bool = True) -> REMOut:
        ...
        AA = batch["AA"]
        CG = batch["CG"]
        step_index = _get_step_index(batch)        # --- Ensemble averages (support reweighting) ---
        ...
        # AA: REM gradient only needs <dU/dλ>_AA, so we can always use mode="avg".
        # If AA_dist is not dict-like, fall back to the original serial path.
        if parallel_dUdL and isinstance(AA_dist, dict):
            ...
            # Per-frame array is not needed for REM (unless you want extra diagnostics).
            dUdL_AA_frame = None
        else:
            dUdL_AA_frame = dUdLByFrame(self.potential, AA_dist)
            dUdL_AA = dUdL(dUdL_AA_frame, w_AA)

        # CG: if Hessian is needed, we must materialize dUdL_CG_frame (for <g_j g_k> term).
...
class MSETrainerAnalytic(BaseTrainer):
    ...
    def make_batch(
        ...
        CG_weight
            ...
            ``CG_bin_idx_frame``.
        weighted_gauge
            If True, the PMF gauge shift is computed using CG bin probabilities;
            otherwise an unweighted mean shift is used.
        step_index
            Logging step counter.
    ...
    def step(self, batch: MSEBatch, apply_update: bool = True) -> MSEOut:
        ...
        step_index = _get_step_index(batch)        # --- Compute dU/dλ for CG (optionally in parallel) ---
        w_CG = CG.get("weight", None)
...
class MultiTrainerAnalytic(BaseTrainer):
    ...
    STEP_SCHEMA: Dict[str, Any] = {
        "batches": "required Sequence[dict]; length == len(trainers); batches[i] must satisfy trainers[i].BATCH_SCHEMA. Each sub-batch may optionally include {parallel_dUdL, dUdL_n_parts, dUdL_n_workers} to enable frame-parallel dUdL inside REM/MSE trainers.",
    ...
    def make_batches(
        *batches: Dict[str, Any],
        ...
        In addition, this helper can **inject frame-parallel dUdL settings**
        ...
        *batches
        ...
        if (parallel_dUdL is None) and (dUdL_n_parts is None) and (dUdL_n_workers is None):
            return out
